...
async function getMakerAddress() {
  if (!makerList.values || !makerList.values.length) {
    await getMaker();
  }
  return makerList.values;
}
async function getChainLp() {
  ...
  data.map((v) => {
    ...
    const c2item = data.find(
      (item) => item.pair.destChain == v.pair.sourceChain
    )
    let lp = {
      makerAddress: v.maker.owner,
      c1ID: v.pair.sourceChain,
      c2ID: v.pair.destChain,
      c1Name: chain2icon(Number(v.pair.sourceChain)),
      c2Name: chain2icon(Number(v.pair.destChain)),
      t1Address: v.pair.sourceToken,
      t2Address: v.pair.destToken,
      tName: tokenType.symbol,
      c1MinPrice: web3.utils.fromWei(v.minPrice, 'ether'),
      c1MaxPrice: web3.utils.fromWei(v.maxPrice, 'ether'),
      c2MinPrice: c2item ? web3.utils.fromWei(c2item.minPrice, 'ether') : null,
      c2MaxPrice: c2item ? web3.utils.fromWei(c2item.maxPrice, 'ether') : null,
      precision: v.sourcePresion,
      c1AvalibleDeposit: 1000,
      c2AvalibleDeposit: 1000,
      c1TradingFee: web3.utils.fromWei(v.tradingFee, 'ether'),
      c2TradingFee: c2item
        ? web3.utils.fromWei(c2item.tradingFee, 'ether')
        : null,
      c1GasFee: web3.utils.fromWei(v.gasFee, 'ether'),
      c2GasFee: c2item ? web3.utils.fromWei(c2item.gasFee, 'ether') : null,
      c1AvalibleTimes: [
        {
...
export { getMakerInfo, getMakerTokenNames, getAllMakerList, getMakerAddress };